...
if (!function_exists('input')) {
    ...
    function input($key = '', $default = null, $filter = ''){
        ...
        $key=str_replace("[]","",$key);
        if($key){
            $value=($headers[$key] ?? $_GET[$key] ?? $_POST[$key] ?? $Hmparameters[$key] ?? $mypost[$key] ?? $default);
            if(is_numeric($value)){
                if(strstr($value,".")){
            ...
            return array_merge([],(array)$Hmparameters,(array)$headers,(array)$_GET,(array)$_POST,(array)$mypost);
        }
    }
}
